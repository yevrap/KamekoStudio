...
- [x] A data module is the only place a shelf entry is declared: adding an item is one
      entry, and the page adds no markup for it.
- [x] Each entry renders a card with a status tag, a title, one sentence, the iteration it
...
- [x] A KILLED entry renders dimmed and is not a link; it stays on the shelf.
- [x] The shelf with no entries renders an honest empty state saying there is nothing on it
      yet — no invented items ship.
- [x] The grid is one column below 520px, two below 900px, three above.
- [x] A pulse line at the top names the current iteration and what it did, in one sentence,
      from the same data module.
- [x] The pulse line's iteration number matches the newest iteration in the repository,
...
- **Tested by:**
  - 47 unit tests in `tests/studio/`, 16 of them new (the suite stood at 31): each declared status; an unrecognised
