...
|---|---|---|---|---|---|
| 1 | SHS-055 | The studio checks tell studio commits from arcade commits on the shared `main` | process | M | Inbox 2026-09-22, blocks the next gate · backlog #1 |
| 2 | SHS-056 | River Run is forked into `studio/games/river-run/` with only `studio_` saves | feature | M | Direction E1 · chat focus 2026-09-22 (Q11 → River Run) · backlog #2 |
| 3 | SHS-057 | The 3D landing page's River Run portal opens the studio fork | feature | S | Direction E1 done-when · Q4 · backlog #3 |
| — | SHS-058 | This iteration's record | docs | S | Ceremony commits |

Three planned tickets, at the cap. One of them (SHS-055) is work on the studio's own
machinery, which uses up this sprint's allowance of one.

**Order.** SHS-055 first. Until it lands, `path-guard`, `production-unchanged` and
`commit-lint` count the arcade commits made since `studio-iteration-04` as studio
violations, so no gate can pass. SHS-056 next, then SHS-057, because the portal needs
...
- A general fork script. SHS-056 writes the procedure down in the handbook so the second
