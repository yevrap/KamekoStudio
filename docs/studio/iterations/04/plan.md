...
|---|---|---|---|
| [SHS-050](tickets/SHS-050-checks-every-push-can-trust.md) | Every push is checked by one stage before it and one after, and the after-stage cannot pass on a stale build or a vacuous baseline | M | QA Engineer |
| [SHS-051](tickets/SHS-051-production-fixes-under-full-process.md) | The studio may fix production under the full process, and the path guard admits a production file only for the ticket that names it, in its own iteration | M | Tech Lead / Architect |
| [SHS-052](tickets/SHS-052-td-009-fixed.md) | Black Hole in One no longer throws when Explore starts with spiral particles alive (TD-009) | S | Front-end / Gameplay Dev |

| ID | Title | Size | Role lead |
|---|---|---|---|
| [SHS-053](tickets/SHS-053-iteration-record.md) | This iteration's record | S | Technical Writer / Learning Lead |

Three committed tickets against the standing cap of 2–3. [SHS-053](tickets/SHS-053-iteration-record.md) is the iteration's own
paperwork, carried as a ticket so the commits that write it have an ID to name — the same
arrangement as [SHS-046](../03/tickets/SHS-046-iteration-record.md) in iteration 03.

**Order.** [SHS-053](tickets/SHS-053-iteration-record.md) lands its first commit first: creating this directory stales the realm's
pulse line. [SHS-050](tickets/SHS-050-checks-every-push-can-trust.md) next, because this iteration pushes every finished ticket and every
push runs its post-deploy stage — with TD-010 open, each of those would report a correct
deploy as a failure until someone waited and ran it again. [SHS-051](tickets/SHS-051-production-fixes-under-full-process.md) before [SHS-052](tickets/SHS-052-td-009-fixed.md), because
[SHS-052](tickets/SHS-052-td-009-fixed.md) edits production files and the guard must admit them by the rule [SHS-051](tickets/SHS-051-production-fixes-under-full-process.md) writes,
not by a one-off exception. [SHS-052](tickets/SHS-052-td-009-fixed.md) last.
...
- **Work in flight is visible.** The executive asked that progress be saved and on the
  ...
  are committed, like a finished ticket. [SHS-053](tickets/SHS-053-iteration-record.md) writes that into `process.md`.
...
|---|---|---|
| May the studio change production files? | **Yes, as fixes, under the full sprint process** — plan, ticket, two-round review, gate, retrospective. A standing permission from the executive, to be written into the handbook by a decision record before it is first used | [SHS-051](tickets/SHS-051-production-fixes-under-full-process.md) writes it down and makes the guard enforce it; [SHS-052](tickets/SHS-052-td-009-fixed.md) is the first use |
...
All three committed tickets are debt or process work: [SHS-050](tickets/SHS-050-checks-every-push-can-trust.md) closes TD-010 and TD-011,
[SHS-052](tickets/SHS-052-td-009-fixed.md) closes TD-009, and [SHS-051](tickets/SHS-051-production-fixes-under-full-process.md) writes down the rule the next production fix depends
...
| Risk | Response |
|---|---|
| TD-009 fails the full suite before [SHS-052](tickets/SHS-052-td-009-fixed.md) lands | Iteration 03's rule stands: a failure with TD-009's exact signature — `Cannot read properties of null (reading 'x')`, uncaught, in a Black Hole in One test that enters Explore — is run again once and both results go in `log.md`. Any other failure is red, full stop. Nothing is exempted or retried automatically. It happened at this iteration's preflight |
...
- **The repository's root `README.md`.** It is one line, and the hygiene audit is right
  about it. It is production documentation, so it goes through [SHS-051](tickets/SHS-051-production-fixes-under-full-process.md)'s rule — first in
...
- **TD-001, TD-003, TD-005, TD-006, TD-007, TD-008.** Unchanged. TD-005 and TD-006 become
  eligible as production fixes once [SHS-051](tickets/SHS-051-production-fixes-under-full-process.md) lands; neither is taken here.
