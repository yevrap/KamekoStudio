...
|---|---|---|---|
| SHS-043 | New tickets are named `SHS-NNN`; the `SS-` series closes at 042 | S | Tech Lead |
| SHS-044 | TD-009 diagnosed: root cause, a deterministic reproduction, the production fix specified | S | QA Engineer |
| SHS-045 | Every ticket a commit names has exactly one ticket file | S | Tech Lead |

| ID | Title | Size | Role lead |
|---|---|---|---|
| SHS-046 | This iteration's record | S | Technical Writer |

Three committed tickets against the standing cap of 2–3. SHS-046 is the iteration's own
...
**Order.** SHS-046 lands its first commit first, because creating this directory stales the
realm's pulse line (see *Files this plan invalidates*). SHS-043 next, because the other two
tickets are named under the convention it introduces and SHS-045 parses it. SHS-045 then,
because it depends on SHS-043's ID grammar. SHS-044 is independent.
...
| Whether `docs-current` earns its cost | **Kept.** It rests on declaring each thing once | SHS-045 extends it in that same spirit rather than adding a parallel check |
...
| The ticket prefix | **`SHS-NNN`, numbering continuing from 043**, decided by the executive; existing `SS-` tickets keep their names | SHS-043, ADR-0006 |

## Reserved capacity

SHS-044 and SHS-045 are the debt share — two of three committed tickets, well over a fifth.
SHS-044 works an open register row (TD-009); SHS-045 closes a gap found while planning this
...
- `studio/shelf-data.js` — `PULSE.iteration` is tied by a test to the newest iteration
  directory, so creating `iterations/03/` stales it. Updated in SHS-046's first commit.
...
| Risk | Response |
|---|---|
| The flaky production test fails the gate for a reason this iteration did not cause | SHS-044 finds the cause, so a red `full-suites` can be read: the failure has one exact signature. A failure with that signature is re-run once and both results are recorded in the log; any other failure is red, full stop. Nothing is exempted or retried automatically |
...
| The new ticket-file rule rejects history the studio cannot change | SHS-045 backfills the three missing ticket files from their commits, in their own iteration's directory, before the rule lands on `main` |
