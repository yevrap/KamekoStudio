...
|---|---|---|---|---|---|
| 1 | [SHS-059](tickets/SHS-059-ticket-mentions-link-to-tickets.md) | Every `SHS-NNN` mention in the repo's Markdown links to its ticket file, and a script keeps it so | process | M | Chat focus, sprint 06 plan · backlog #32 |
| 2 | [SHS-060](tickets/SHS-060-river-run-power-ups.md) | River Run fork: a shield and rapid-fire / spread shot float on the river | feature | M | Direction E1 (sprint 06: the first experiment) · Q12 ⭐ · backlog #4 |
| 3 | [SHS-061](tickets/SHS-061-river-run-tone-start-time.md) | River Run fork: the music never schedules a Tone.js start below `Tone.now()` | fix | S | [SHS-056](../05/tickets/SHS-056-river-run-fork.md) build · iteration 05 review · backlog #22 |
| — | [SHS-062](tickets/SHS-062-iteration-record.md) | This iteration's record | docs | S | Ceremony commits |

Three planned tickets, at the cap. [SHS-059](tickets/SHS-059-ticket-mentions-link-to-tickets.md) is work on the studio's own machinery (its
...
**Order.** [SHS-059](tickets/SHS-059-ticket-mentions-link-to-tickets.md) first, because it is what the executive asked for. [SHS-060](tickets/SHS-060-river-run-power-ups.md) next: it is
E1's first gameplay experiment, and it retires the fork's byte-for-byte equality test.
[SHS-061](tickets/SHS-061-river-run-tone-start-time.md) last, because the audio exemption it removes rests on that equality test. Each is
one build session.
...
([SHS-043](../03/tickets/SHS-043-ticket-prefix.md)…058) sit in 52 Markdown files, mostly under `docs/studio/`, plus two arcade
...
| Retiring the equality test loses the record of how the fork differs from production | The ticket keeps `EDITS` and the source commit as the record of the copy, with a note in the test saying why equality stopped at [SHS-060](tickets/SHS-060-river-run-power-ups.md) |
