...
**Next:** `build SHS-074` — the skill's build and review steps describe the branch and pull-request flow
...
- [SHS-074](../iterations/09/tickets/SHS-074-skill-describes-pull-requests.md) is the sprint's one studio-machinery ticket and is built on trunk, like
  this sprint's others: the pull-request flow it writes starts at plan 10 (#50 rides on
  sprint 10's first games ticket). Its skill edits need a ticketed commit (ADR-0011 §6).
- Samovar at review: the brim is 250 ms free, 200 ms at a star (a drip, `data-overflow`
  ...
  near 0 and whether players watch the glass or count seconds (SHS-072).
- Screenshots: save to disk, compose into one image, look once (retro 08). SHS-073 looked
  at four sheets while tuning the drip's drawing; retro 09 checks that, review's writes
  ($3.43 in 08) and the Playtester's writes and reads ($1.97).
