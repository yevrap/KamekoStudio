...
| p0-07 | Add unit tests for durak-dungeon and durak-tactics pure logic | M | open | Zero test coverage for either game. `games/durak-dungeon/constants.js` + `state.js` have pure, DOM-free logic directly analogous to what's already tested elsewhere: `canDefend` (combat rule incl. `no-trumps` mutation branch), `mulberry32`/`seedFromString` (seeded-run RNG), `hasRelic`, `getActiveTrumpSuit`, `shuffle`. Found during 2026-07-07 /improve scan. |
