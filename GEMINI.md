...
- **Three input modes:** Tap to Spell (T9 scroll-select, 820ms auto-confirm), T9 Smart (guided typing), Keyboard. Mode persisted in `keypadQuest_inputMode`. The mode selector buttons (`.mode-btn`) are **not in the game HTML** — they are injected into the settings panel by `injectInputModeSection()` on `settingsOpened` (above the stats section, below gallery link) and removed on `settingsClosed`. Click events use document-level delegation (`e.target.closest('.mode-btn')`). `setInputMode()` updates active state via `querySelectorAll('.mode-btn')` as before.
...
- **`autoSubmitTimer`:** Single shared `setTimeout` handle used by both T9 Smart and Tap to Spell auto-submit paths. Cleared in `showNextPrompt()`, the `⌫` back key handler, and the `✓` ok handler. **NOT cancelled on mode switch** — the closure captures the answer at schedule time so it fires correctly regardless of current mode. In scroll mode, `autoFillScrollSpecials` does NOT pre-clear `t9buf` — it stays populated until the timer fires so the ok handler can read it synchronously.
- **Mode switching preserves input progress:** `setInputMode()` converts typed progress across modes — `t9buf.length` (scroll) / `t9pos` (predict) / `ki.value` prefix match (keyboard) — so switching modes mid-answer doesn't lose work. Pending scroll char (`t9pend`) is always cleared. `t9timer` (820ms scroll auto-confirm) is cancelled on mode switch.
- **T9_MAP key `0`:** Not in `T9_MAP` — pressing `0` always types the digit `0` directly. There is no SPC cycling.
- **Keyboard mode:** Uses a near-invisible fixed `<input>` (`1×1px`, `opacity:0.01`) at viewport center so focusing it never causes scroll/layout shift on mobile. `#keyboard-wrap` shows a `#keyboard-focus-btn` (dashed button, "Tap to type ▸" / "typing... ▸") and a Submit button. Tapping anywhere in `#input-area` re-focuses the hidden input. Display updates via `input` event for zero-lag feedback. Re-focuses on Enter submit and on `visibilitychange` (iOS bfcache). **Submit handlers capture `ki.value` then clear it BEFORE calling `submitAnswer()`** so that `updateInputDisplay()` inside submitAnswer/showNextPrompt always sees an empty field. `showNextPrompt()` also explicitly clears `ki.value` when in keyboard mode to handle all advance paths (skip, autoSubmitTimer from a prior mode, wave end).
