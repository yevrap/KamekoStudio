...
export function handleTap(wx, wy) {
    if (!S.inventory.orbitMagnet?.enabled) return;
    ...
    if (S.phase === 'flight') {
        const gap = dist(comet.x, comet.y, hit.x, hit.y) - (hit.r + COMET_R);
        if (gap >= ORBIT_MIN_GAP && gap <= ORBIT_MAX_GAP) {
            beginDescentFromFlight(hit);
        }
