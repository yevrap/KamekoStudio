...
// not run is never silently dropped: the gate stage treats "not run" as a
// failure, because a guarantee nobody verified is not a guarantee.
//
...
function usage() {
  ...
  --base=<ref>           git ref the path guard and commit lint diff against
                         (default: the latest studio-iteration tag)
  ...
  --marker=<text>        string that must appear on the deployed studio page
...
async function main() {
  ...
  const ctx = {
    root: ROOT,
    ...DEFAULTS,
    base: opts.base ?? latestIterationTag(ROOT) ?? 'origin/main',
  ...
  // "not run" is a failure at the gate: the gate exists to be conclusive.
  for (const r of results) {
    if (r.stage === 'gate' && r.status === 'skip') {
      r.status = 'fail';
      r.detail = `not run, and the gate cannot pass on an unverified check — ${r.detail}`;
    }
  }
