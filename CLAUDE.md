...
- **`autoSubmitTimer`:** Single shared `setTimeout` handle used by both T9 Smart and Tap to Spell auto-submit paths. Cleared (cancelled) in `setInputMode()`, `showNextPrompt()`, the `⌫` back key handler, and the manual `✓` ok handler. In scroll mode, `autoFillScrollSpecials` does NOT pre-clear `t9buf` — it stays populated until the timer fires so the ok handler can read it synchronously. The ok handler cancels any timer set by `confirmScrollChar` (which calls `autoFillScrollSpecials`) by placing its `clearTimeout` call after `confirmScrollChar()`.
